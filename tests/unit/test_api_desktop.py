...
def test_desktop_status_uses_absolute_websocket_path(tmp_path, monkeypatch):
    monkeypatch.setattr(api, "API_KEY", "secret")
    monkeypatch.setattr(api, "_vnc_ready", lambda _host, _port: True)
    monkeypatch.setenv("ENABLE_NOVNC", "true")
    monkeypatch.setenv("NOVNC_WEB_DIR", str(tmp_path))
    ...
    status = api.get_desktop_status(_request("secret", forwarded_proto="https"))
    path = parse_qs(urlsplit(status["url"]).query)["path"][0]

    assert path.startswith("/api/desktop/ws/")
    assert not path.startswith("/desktop/api/")
